...
class CommonEventEmitter extends EventEmitter {
    ...
    emitEvent(event, ...params) {
        ...
        if (typeof params[0] !== 'object') {
            throw new Error('Oops, if you pass event params, the first param must be an object');
        }
        else {
    ...
    registerSubscriber(event, subscriber) {
        const subscriberRegistered = this.listeners().find((s) => String(s.name) === String(subscriber.name));
